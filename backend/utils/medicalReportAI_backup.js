...
Extract every single test parameter found in the report. Here's what I can see in this report:
...

{
  "testResults": [
    {
      ...
      "normalRange": {
        "min": "",
        "max": "200",
        "description": "<200: Desirable, 200-239: Borderline risk, >240: High risk"
      },
      "status": "normal",
  ...
  "aiAnalysis": {
    "summary": "Comprehensive metabolic panel shows mostly normal results with some areas needing attention.",
    "keyFindings": [
      {
        "parameter": "HbA1c",
        "value": "6.3%",
        "status": "high",
        "description": "Indicates pre-diabetic range, requires dietary monitoring"
      }
    ],
    "recommendations": [
    ...
    "overallAssessment": "Generally good health with attention needed for glucose control",
    "urgencyLevel": "medium"
  },
  "extractedMetadata": {
...
7. Do not use markdown formatting`;
  ...
  parseAIResponse(responseText) {
    try {
      ...
      try {
        const fallbackData = this.extractDataFallback(responseText);
        if (fallbackData.testResults.length > 0) {
  ...
  extractDataFallback(responseText) {
    // Simple fallback extraction for common patterns
    const testResults = [];
    
    // Try to extract basic patterns from the response
    const lines = responseText.split('\n');
    for (const line of lines) {
      if (line.includes('parameter') && line.includes('value')) {
        // This is a basic pattern matcher - could be enhanced
        continue;
      }
    }
    ...
    return {
      testResults,
      aiAnalysis: {
        summary: "Partial extraction attempted. Manual review recommended.",
        keyFindings: [],
        recommendations: ["Manual review recommended"],
        followUpActions: ["Consult healthcare provider"],
        riskFactors: [],
    ...
    try {
      ...
      for (const currentResult of currentResults) {
        const previousResult = latestPrevious.testResults.find(
          prev => prev.parameter.toLowerCase() === currentResult.parameter.toLowerCase()
        );
  ...
  calculateTrend(current, previous) {
    try {
      ...
      if (Math.abs(changePercent) > 5) { // 5% threshold for significant change
        if (changePercent > 0) {
          trendDirection = current.status === 'normal' ? 'improving' : 'declining';
  ...
  extractNumericValue(valueString) {
    try {
      // Extract first number from string (handles cases like "120 mg/dL", "5.2", etc.)
