...
3. DO NOT include "trends" array in response (it will be calculated separately)
4. Ensure all numeric values are properly formatted
5. Only include parameters that have actual values
...
7. Do not use markdown formatting`;
  ...
  validateAndClean(analysis) {
    const validated = {
      testResults: [],
      aiAnalysis: {
        summary: analysis.aiAnalysis?.summary || 'No summary available',
        keyFindings: analysis.aiAnalysis?.keyFindings || [],
        recommendations: analysis.aiAnalysis?.recommendations || [],
        followUpActions: analysis.aiAnalysis?.followUpActions || [],
        riskFactors: analysis.aiAnalysis?.riskFactors || [],
        overallAssessment: analysis.aiAnalysis?.overallAssessment || 'Assessment pending',
        urgencyLevel: this.validateUrgencyLevel(analysis.aiAnalysis?.urgencyLevel)
      },
      trends: []
    };

    // Validate test results
    validated.testResults = (analysis.testResults || []).map(result => ({
    ...
    // Validate trends and filter out any with NaN values
    if (analysis.trends && Array.isArray(analysis.trends)) {
      validated.trends = analysis.trends
        .filter(trend => trend && typeof trend === 'object')
        .map(trend => ({
          parameter: trend.parameter || 'Unknown',
          trend: this.validateTrendDirection(trend.trend),
          description: trend.description || 'No description available',
          previousValue: trend.previousValue || 'N/A',
          changePercent: this.validateChangePercent(trend.changePercent)
        }))
        .filter(trend => !isNaN(trend.changePercent) && isFinite(trend.changePercent));
    }
